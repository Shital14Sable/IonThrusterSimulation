function grid_points = Grid_pnt_Calc(apts,grids,gridThick,AptDiam,Gap,lengthX, lengthY,res, voltage, Test)
...
for i = 1:grids*2-1
    if mod(i,2)==0 %If we just added a grid thickness, then add a gap thickness
        x(i+1) = x(i)+(Gap(m)).*res;
    else %If we just added a gap, then add a grid thickness
