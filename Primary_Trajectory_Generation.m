function [NPos_x, NPos_y, time_step, Vx_new, Ex, Ey] = Primary_Trajectory_Generation(V, sim_data, thrstr_data, folder_name, Grid, mesh_itr)
...
if isfolder(folder_name)
    writematrix(Ex, [folder_name '\Test_conv_' num2str(mesh_itr) 'Ex.csv'])
    writematrix(Ey, [folder_name '\Test_conv_' num2str(mesh_itr) 'Ey.csv'])
    writematrix(NPos_x, [folder_name '\Test_conv_' num2str(mesh_itr) 'NPos_x.csv'])  % writes the generated Trajectory matrix to a given name
    writematrix(NPos_y, [folder_name '\Test_conv_' num2str(mesh_itr) 'NPos_y.csv'])  % writes the generated Trajectory matrix to a given name
    writematrix(time_step, [folder_name '\Test_conv_' num2str(mesh_itr) 'TimeStep.csv'])  % writes the generated Time Step matrix to a given name
    writematrix(Vx_new, [folder_name '\Test_conv_' num2str(mesh_itr) 'Vx.csv'])  % writes the generated Velocity matrix to a given name
else
